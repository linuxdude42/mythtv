...
#ifdef INCLUDE_TCP_SERVER
    s_tcp_thread->terminate();
#endif
    s_unix_thread->terminate();
    QFile::remove(s_unix_sockname);
...
{
    QFETCH(QString, qs_in);
    QFETCH(QString, qs_expected);
...
std::array<const char *,17>parse_string_strings
{
    ...
    /*  1 */ "Lorem \\a\\b\\e dolor \\f\\n\\r sit",
    /*  2 */ "Lorem \a\b\e dolor \f\n\r sit",

    /*  3 */ "Lorem \\t\\v\\\n dolor",
    /*  4 */ "Lorem \t\v",

    /*  5 */ "Lorem\\40ipsum\\040 dolor \\133sit\\135",
    /*  6 */ "Lorem\\x20ipsum\\x20 dolor \\x5bsit\\x5D",
    /*  7 */ "Lorem ipsum  dolor [sit]",

    /*  8 */ "Lorem \\A\\B\\V\\Z dolor \\[sit\\]",
...
void TestLirc::test_parse_flags(void)
{
    ...
    uint actual = lirc_flags(&l_state, text.toUtf8().data());
...
{
    QFETCH(QString, qs_home);
    QFETCH(QString, qs_user_file);
    QFETCH(QString, qs_filename);
    QFETCH(QString, qs_current);
    QFETCH(QString, qs_expected);
    ...
    lirc_state l_state {};
    l_state.lirc_prog = strdup("__FILE__");
    l_state.lircrc_user_file = strdup(qs_user_file.toUtf8().data());
...
void TestLirc::test_open_data(void)
{
    ...
    QString filename1 = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc1.txt";
    QString filename2 = "/" + filename1; // sic
...
{
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc1.txt";
    const std::string ss_filename = filename.toStdString();
    ...
    QString output = QString("#!%1\ninclude %2\n")
        .arg(expected_sha_bang.data(), ss_filename.data());
    tmpfile.write(output.toUtf8());
    tmpfile.close();
    const std::string ss_tmpfilename = tmpfile.fileName().toStdString();
    ...
    QCOMPARE(full_name, ss_tmpfilename);
...
void TestLirc::test_readconfig_internal2(void)
{
    ...
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc2.txt";
    const std::string ss_filename = filename.toStdString();
    std::string full_name;
    std::string sha_bang;
    ...
    QCOMPARE(full_name, ss_filename);
...
void TestLirc::test_readconfig_internal3(void)
{
    ...
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc3.txt";
    const std::string ss_filename = filename.toStdString();
...
void TestLirc::test_readconfig_internal4(void)
{
    ...
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc4.txt";
    const std::string ss_filename = filename.toStdString();
...
void TestLirc::test_readconfig_internal5(void)
{
    ...
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc5.txt";
    const std::string ss_filename = filename.toStdString();
...
{
    QString qs_command { "/bin/echo make some noise" };
    std::string ss_command = qs_command.toStdString();

    // Write temporary config file.  The name must be the same as the
    // lircd socket name, minus the final 'd'.
    QString qs_conf_name = s_unix_sockname.left(s_unix_sockname.size()-1);
...
{
    QString qs_target_file = s_unix_sockname + ".rc7";
    QString qs_command = QString("touch %1").arg(qs_target_file);

    // Write temporary config file.  This needs to be an invalid name
    // to force lirc_readconfig to try and start the lircd daemon.
    QString qs_conf_name = s_unix_sockname.left(s_unix_sockname.size()-2);
    std::string ss_conf_name = qs_conf_name.toStdString();
    QFile conf_file = QFile(qs_conf_name);
    QVERIFY(conf_file.open(QIODevice::ReadWrite | QIODevice::Text));
    ...
    bool exists = QFile::exists(qs_target_file);
...
void TestLirc::test_code2char_internal(void)
{
    ...
    QString filename = QStringLiteral(TEST_SOURCE_DIR) + "/data/lirc_full.txt";
    const std::string ss_filename = filename.toStdString();
    ...
    std::string button = strdup("purple");
    int level = lirc_iscode(entry, remote, button, 0);
    QCOMPARE(level, 0);
...
void TestLirc::test_getsocketname(void)
{
    ...
    size_t len = lirc_getsocketname(input.data(), addr.sun_path, sizeof(addr.sun_path));
