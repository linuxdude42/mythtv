...
static void lirc_printf(const struct lirc_state *state, const char *format_str, ...)
{
	...
	if(state)
	{
		if (!state->lirc_verbose)
			return;
		lformat = state->lirc_prog + ": " + format_str;
                       ...
                       char **full_name)
{
	...
	else if(fin==nullptr)
	{
		...
		{
			lirc_printf(state, "could not open config files "
				    "%s and %s\n",
...
std::string lirc_ir2char(const struct lirc_state *state,struct lirc_config *config,char *code)
{
	...
	if(warning)
	{
		fprintf(stderr,"warning: lirc_ir2char() is obsolete\n");
		warning=0;
	}
...
int lirc_code2charprog(struct lirc_state *state,struct lirc_config *config,char *code,std::string& string,
		       std::string& prog)
{
	std::string backup = state->lirc_prog;
	state->lirc_prog.clear();
...
const std::string lirc_getmode(const struct lirc_state *state, struct lirc_config *config)
{
	if(config->sockfd!=-1)
	{
		static std::array<char,LIRC_PACKET_SIZE> s_buf;
...
const std::string lirc_setmode(const struct lirc_state *state, struct lirc_config *config, const std::string& mode)
{
	if(config->sockfd!=-1)
	{
		static std::array<char,LIRC_PACKET_SIZE> s_buf {};
