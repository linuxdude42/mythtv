                    ...
                    int (check)(char *s))
{
	...
	using unique_char_p = std::unique_ptr<char,decltype(&free)>;
	unique_char_p filename_p(filename, &free);
	unique_char_p sha_bang_p(sha_bang, &free);
	...
	/* launch lircrcd */
	command=static_cast<char*>(malloc(strlen(sha_bang)+1+strlen(filename)+1));
	if(command==nullptr)
		return 0;
	strcpy(command, sha_bang);
	strcat(command, " ");
