...
AudioOutputSettings* AudioOutputJACK::GetOutputSettings(bool /*digital*/)
{
    ...
    int rate = 0;
    std::unique_ptr<AudioOutputSettings> settings { new AudioOutputSettings() };
...
bool AudioOutputJACK::OpenDevice()
{
    ...
    using cleanup_t = std::unique_ptr<AudioOutputJACK, decltype(cleanup_fn)>;
    cleanup_t cleanup { this, cleanup_fn };
    ...
    // Device opened successfully. Skip cleanup function.
    cleanup.release();
    return true;
}
