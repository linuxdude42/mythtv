...
void MusicFileScanner::cleanDB()
{
    ...
    {
        LOG(VB_GENERAL, LOG_ERR, QString("********** %1: deletedCount=%2").arg(__PRETTY_FUNCTION__).arg(deletedCount));
        deletedCount = 0;
        query.seek(-1);
        ...
        while (query.next())
        {
            LOG(VB_GENERAL, LOG_ERR, QString("********** %1 in query.next").arg(__PRETTY_FUNCTION__));
