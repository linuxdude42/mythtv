...
///< \return Returns a pointer to a newly allocated string, or nullptr in case of error.
{
  if (Length > 0 && Data && *Data) {
     ...
     auto s = std::string(reinterpret_cast<const char*>(d), l);
     Length -= d - *Data + l;
     *Data = d + l;
     return s;
     }
...
public:
  ...
  ~cCiApplicationInformation() override = default;
  bool Process(int Length = 0, const uint8_t *Data = nullptr) override; // cCiSession
  bool EnterMenu(void);
  char    *GetApplicationString()       { return strdup(m_menuString.data()); };
     ...
     switch (Tag) {
       ...
       case AOT_ENQ: {
            ...
            if (l > 0) {
               ...
               m_enquiry->m_text = std::string(reinterpret_cast<const char*>(d), l);
               }
            }
            break;
       case AOT_CLOSE_MMI: {
