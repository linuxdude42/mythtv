...
int BiopBinding::Process(const unsigned char *data)
{
    ...
    m_objInfo.resize((data[off] << 8) | data[off + 1]);
    off += 2;

    if (!m_objInfo.empty())
    {
        memcpy(m_objInfo.data(), data + off, m_objInfo.size());
    }
...
bool BiopMessage::ProcessMsgHdr(const unsigned char *data, unsigned long *curp)
{
    ...
    m_objKind.resize(COMBINE32(buf, off));
    off += 4;
    memcpy(m_objKind.data(), buf + off, m_objKind.size());
    off += m_objKind.size();

    m_objInfo.resize(buf[off] << 8 | buf[off + 1]);
    off += 2;
    memcpy(m_objInfo.data(), buf + off, m_objInfo.size());
    off += m_objInfo.size();
        ...
        if (pDir && binding.m_name.m_compCount >= 1)
        {
            if ("fil" == binding.m_name.m_comps[0].m_kind)
                ...
                LOG(VB_DSMCC, LOG_WARNING, QString("[biop] ProcessDir unknown kind %1")
                    .arg(binding.m_name.m_comps[0].m_kind.data()));
        }
    }
...
int BiopTap::Process(const unsigned char *data)
{
    ...
    off += 2;
    m_selector.resize(data[off++]);
    memcpy(m_selector.data(), data + off, m_selector.size());
    if (m_use == 0x0016) // BIOP_DELIVERY_PARA_USE
    {
        unsigned selector_type = (data[off] << 8) | data[off + 1];
        if (m_selector.size() >= 10 && selector_type == 0x0001)
        {
            off += 2;
            ...
            off += 4;
            m_selector.resize(m_selector.size() - 10);
        }
    }
