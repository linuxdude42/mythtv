...
MHOctetString::MHOctetString(const char *str, int nLen)
{
    ...
    // Add a NUL terminator just past the end of the string so that it
    // can be referenced as a C string.  std::string does this by
    // default, but it doesn't officially support unsigned chars that
    // this class needs to support.
    m_pChars.resize(nLen+1);
    m_pChars[nLen+1] = 0;

    m_pChars.resize(nLen);
    if (nLen != 0)
    {
        memcpy(m_pChars.data(), str, nLen);
    }
}
...
MHOctetString::MHOctetString(const unsigned char *str, int nLen)
{
    ...
    // consistency with the prior constructor.
    m_pChars.resize(nLen+1);
    m_pChars[nLen+1] = 0;
...
MHOctetString::MHOctetString(const MHOctetString &str, int nOffset, int nLen)
{
    ...
    // Add a NUL terminator just past the end of the string for
    // consistency with the prior constructors.
    m_pChars.resize(nLen+1);
    m_pChars[nLen+1] = 0;
    ...
    if (nLen != 0)
    {
        memcpy(m_pChars.data(), str.m_pChars.data() + nOffset, nLen);
    }
}
