...
void TestMhegBaseClasses::test_octetstring_int8(void)
{
    ...
    QCOMPARE(s.Size(), 50L);
    auto* bytes = reinterpret_cast<const char*>(s.Bytes());
...
{
    const uint8_t ustr1[] { // "Lorem ipsum.\0Id."
        0x4C, 0x6F, 0x72, 0x6E, 0x6D, 0x20, 0x69, 0x70,
        0x73, 0x75, 0x6D, 0x2E, 0x00, 0x4A, 0x64, 0x2E };
    const uint8_t ustr2[] { // "Lorem ipsum.\0Ut."
        0x4C, 0x6F, 0x72, 0x6E, 0x6D, 0x20, 0x69, 0x70,
        0x73, 0x75, 0x6D, 0x2E, 0x00, 0x55, 0x74, 0x2E };
    const uint8_t ustr3[] { // "Lorem ipsum!"
        0x4C, 0x6F, 0x72, 0x6E, 0x6D, 0x20, 0x69, 0x70,
        0x73, 0x75, 0x6D, 0x21 };

    MHOctetString s1 {ustr1, 16};
    MHOctetString s2 {ustr2, 16};
    MHOctetString s3 {ustr3, 12};
    
    QCOMPARE(s1.Size(), 16L);
    auto* bytes = reinterpret_cast<const char*>(s1.Bytes());
    int len = strlen(bytes);
    QCOMPARE(len, 12);  // Don't use strlen. There can be embedded NUL characters.
    QVERIFY(memcmp(bytes, ustr1, 16) == 0);
