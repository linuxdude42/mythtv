...
# **updates**
> InlineResponse20043 updates(since)
...
from openapi_client.api import updates_api
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = updates_api.UpdatesApi(api_client)
    since = 3.14 # float | 

    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.updates(since)
        pprint(api_response)
    except openapi_client.ApiException as e:
        print("Exception when calling UpdatesApi->updates: %s\n" % e)
```
