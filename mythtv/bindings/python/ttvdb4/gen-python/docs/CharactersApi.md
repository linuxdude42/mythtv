...
# **get_character_base**
> InlineResponse20010 get_character_base(id)
...
from openapi_client.api import characters_api
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    api_instance = characters_api.CharactersApi(api_client)
    id = 1 # int | id

    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.get_character_base(id)
        pprint(api_response)
    except openapi_client.ApiException as e:
        print("Exception when calling CharactersApi->get_character_base: %s\n" % e)
```
